...
export const useNostrBookReader = (): NostrBookReaderUtils => {
  ...
  const fetchBookData = useCallback(
    async (identifier: string | { bookId: string; authorPubkey: string }) => {
      if (!isConnected || !ndk) {
      ...
      try {
        ...
        if (typeof identifier === 'string') {
          ...
          try {
            ...
            throw new Error(
              `Failed to decode identifier: ${e instanceof Error ? e.message : String(e)}`
            );
          }
        } else {
          // Assume object
        ...
        try {
          ...
          throw new Error(
            `Failed to parse book metadata content: ${e instanceof Error ? e.message : String(e)}`
          );
        }
        ...
        for (const ev of chapterEventSet) {
          try {
            ...
            if (!stateId) {
              console.warn(
                `Chapter event ${ev.id} missing stateId in content and d tag. Skipping.`
              );
              continue;
            }

            // Ensure transitions have necessary fields, providing defaults
            const transitions = (chapterContent.transitions ?? [])
              .map(
        ...
        if (fetchedStates[parsedMeta.startStateId]) {
          ...
          console.warn(
            `startStateId "${parsedMeta.startStateId}" not found in fetched chapters. Falling back to first chapter "${firstChapterId}".`
          );
   ...
   */
  const setCurrentChapterById = useCallback(
   ...
   */
  const goToChapterByChoice = useCallback(
    (choiceTextOrId: string): boolean => {
      ...
      const transition = current.transitions.find(
        t => t.choiceText === choiceTextOrId || t.id === choiceTextOrId
      );
      if (transition && transition.targetStateId) {
        return setCurrentChapterById(transition.targetStateId);
      } else {
        console.warn(
          `Transition not found for choice "${choiceTextOrId}" in chapter ${currentChapterId}`
        );
   ...
   */
  const fetchChapterById = useCallback(
    async (chapterId: string): Promise<FsmState | null> => {
      ...
      try {
        console.log(
          `Fetching specific chapter: kind=${NODE_KIND}, author=${currentAuthorPubkey}, #d=${chapterId}`
        );
        const chapterFilter: NDKFilter = {
          kinds: [NODE_KIND],
